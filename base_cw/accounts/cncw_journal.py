...
class AccountJournalGroup(models.Model):
    _name = 'cncw.account.journal.group'
    _description = "日记账组"
    _check_company_auto = True

    name = fields.Char("日记账组", required=True, translate=False)
    company_id = fields.Many2one('res.company', required=True, default=lambda self: self.env.company)
    excluded_journal_ids = fields.Many2many('cncw.account.journal', string="排程", domain="[('company_id', '=', company_id)]",
        check_company=True)
    sequence = fields.Integer(default=10)


class AccountJournal(models.Model):
    _name = "cncw.account.journal"
    _description = "日记账"
    ...
    type_control_ids = fields.Many2many('cncw.account.type', 'cncw_journal_account_type_control_rel', 'journal_id', 'type_id', string='允许科目类别')
    account_control_ids = fields.Many2many('cncw.account', 'cncw_journal_account_control_rel', 'journal_id', 'account_id', string='允许科目',
        check_company=True,
        domain="[('deprecated', '=', False), ('company_id', '=', company_id), ('is_off_balance', '=', False)]")
    default_account_type = fields.Many2one('cncw.account.type', compute="_compute_default_account_type")
    default_account_id = fields.Many2one(
        comodel_name='cncw.account', check_company=True, copy=False, ondelete='restrict',
        string='默认科目',
    ...
    payment_debit_account_id = fields.Many2one(
        comodel_name='cncw.account', check_company=True, copy=False, ondelete='restrict', string='未付款科目',
    ...
    payment_credit_account_id = fields.Many2one(
        comodel_name='cncw.account', check_company=True, copy=False, ondelete='restrict', string='Outstanding Payments Account',
        domain=lambda self: "[('deprecated', '=', False), ('company_id', '=', company_id), \
                             ('user_type_id.type', 'not in', ('receivable', 'payable')), \
                             '|', ('user_type_id', '=', %s), ('id', '=', default_account_id)]" % self.env.ref('account.data_account_type_current_assets').id)
    suspense_account_id = fields.Many2one(
        comodel_name='cncw.account', check_company=True, ondelete='restrict', readonly=False, store=True,
        compute='_compute_suspense_account_id',string='暂估科目',
        domain=lambda self: "[('deprecated', '=', False), ('company_id', '=', company_id), \
                             ('user_type_id.type', 'not in', ('receivable', 'payable')), \
                             ('user_type_id', '=', %s)]" % self.env.ref('account.data_account_type_current_liabilities').id)
    restrict_mode_hash_table = fields.Boolean(string="已过账条目",)
    sequence = fields.Integer(string="序号",default=10)

    invoice_reference_type = fields.Selection(string='发票类型', required=True, selection=[('none', 'Free'), ('partner', 'Based on Customer'), ('invoice', 'Based on Invoice')], default='invoice', help='You can set here the default communication that will appear on customer invoices, once validated, to help the customer to refer to that particular invoice when making the payment.')
    invoice_reference_model = fields.Selection(string='发票标准', required=True, selection=[('china', '中国发票')], default='china',)

    currency_id = fields.Many2one('res.currency', string="货币")
    company_id = fields.Many2one('res.company', string='公司', required=True, readonly=True, index=True, default=lambda self: self.env.company,
        help="Company related to this journal")
    country_code = fields.Char(related='company_id.country_id.code', readonly=True)

    refund_sequence = fields.Boolean(string='通知', default=False)
    ...
    inbound_payment_method_ids = fields.Many2many(
        comodel_name='account.payment.category',
        relation='cncw_cncw_account_journal_inbound_payment_method_rel',
        column1='journal_id',
    ...
    outbound_payment_method_ids = fields.Many2many(
        comodel_name='account.payment.category',
        relation='cncw_cncw_account_journal_outbound_payment_method_rel',
        column1='journal_id',
    ...
    profit_account_id = fields.Many2one(
        comodel_name='cncw.account', check_company=True,
    ...
    company_partner_id = fields.Many2one('res.partner', related='company_id.partner_id', string='往来单位', readonly=True, store=False)
    ...
    bank_statements_source = fields.Selection(selection=_get_bank_statements_available_sources, string='Bank Feeds', default='未定义',)
    ...
    sale_activity_type_id = fields.Many2one('mail.activity.type', string='安排', default=False, help="Activity will be automatically scheduled on payment due date, improving collection process.")
    sale_activity_user_id = fields.Many2one('res.users', string="用户", help="Leave empty to assign the Salesperson of the invoice.")
    sale_activity_note = fields.Text('摘要')

    # alias configuration for journals
    alias_id = fields.Many2one('mail.alias', string='邮件', copy=False)
    alias_domain = fields.Char('域名', compute='_compute_alias_domain', default=_default_alias_domain, compute_sudo=True)
    alias_name = fields.Char('别名', copy=False, related='alias_id.alias_name',  readonly=False)

    journal_group_ids = fields.Many2many('cncw.account.journal.group',
    ...
    _sql_constraints = [
        ('code_company_uniq', 'unique (code, name, company_id)', '日记账的代码和名称在每个公司必须是唯一 !'),
    ]
    ...
    def _compute_suspense_account_id(self):
        for journal in self:
            ...
            elif journal.company_id.cncw_account_journal_suspense_account_id:
                journal.suspense_account_id = journal.company_id.cncw_account_journal_suspense_account_id
            else:
                journal.suspense_account_id = False
    ...
    def _constrains_type_control_ids(self):
        self.env['cncw.invoice.move.line'].flush(['account_id', 'journal_id'])
        self.flush(['type_control_ids'])
        self._cr.execute("""
            SELECT aml.id
            FROM cncw_invoice_move_line aml
            ...
            AND NOT EXISTS (SELECT 1 FROM cncw_account acc
    ...
    def _constrains_account_control_ids(self):
        ...
        self._cr.execute("""
            ...
            FROM cncw_invoice_move_line aml
            WHERE aml.journal_id in (%s)
            AND EXISTS (SELECT 1 FROM cncw_journal_account_control_rel rel WHERE rel.journal_id = aml.journal_id)
            AND NOT EXISTS (SELECT 1 FROM cncw_journal_account_control_rel rel WHERE rel.account_id = aml.account_id AND rel.journal_id = aml.journal_id)
        """, tuple(self.ids))
        if self._cr.fetchone():
    ...
    def _check_company_consistency(self):
        ...
        self._cr.execute('''
            SELECT move.id
            FROM cncw_invoice_move move
            JOIN cncw_account_journal journal ON journal.id = move.journal_id
    ...
    def _update_mail_alias(self, vals):
        ...
        if self.alias_id:
            ...
            alias_values['alias_model_id'] = self.env['ir.model']._get('cncw.invoice.move').id
            alias_values['alias_parent_model_id'] = self.env['ir.model']._get('cncw.account.journal').id
    ...
    def write(self, vals):
        for journal in self:
            ...
            if ('company_id' in vals and journal.company_id.id != vals['company_id']):
                if self.env['cncw.invoice.move'].search([('journal_id', '=', journal.id)], limit=1):
            ...
            if 'restrict_mode_hash_table' in vals and not vals.get('restrict_mode_hash_table'):
                journal_entry = self.env['cncw.invoice.move'].search([('journal_id', '=', self.id), ('state', '=', 'posted'), ('secure_sequence_number', '!=', 0)], limit=1)
    ...
    def get_next_bank_cash_default_code(self, journal_type, company):
        ...
        journals = self.env['cncw.account.journal'].search([('code', 'like', journal_code_base + '%'), ('company_id', '=', company.id)])
    ...
    def _fill_missing_values(self, vals):
        ...
        random_account = self.env['cncw.account'].search([('company_id', '=', company.id)], limit=1)
        ...
        if journal_type in ('bank', 'cash'):
            ...
            if not has_liquidity_accounts:
                default_account_code = self.env['cncw.account']._search_new_account_code(company, digits, liquidity_account_prefix)
                default_account_vals = self._prepare_liquidity_account_vals(company, default_account_code, vals)
                vals['default_account_id'] = self.env['cncw.account'].create(default_account_vals).id
            if not has_payment_accounts:
                vals['payment_debit_account_id'] = self.env['cncw.account'].create({
                    'name': _("Outstanding Receipts"),
                    'code': self.env['cncw.account']._search_new_account_code(company, digits, liquidity_account_prefix),
                    'reconcile': True,
                    'user_type_id': current_assets_type.id,
                    'company_id': company.id,
                }).id
                vals['payment_credit_account_id'] = self.env['cncw.account'].create({
    ...
    def create_invoice_from_attachment(self, attachment_ids=[]):
        ...
        invoices = self.env['cncw.invoice.move']
        for attachment in attachments:
            attachment.write({'res_model': 'mail.compose.message'})
            decoders = self.env['cncw.invoice.move']._get_create_invoice_from_attachment_decoders()
            invoice = False
            for decoder in sorted(decoders, key=lambda d: d[0]):
                ...
                if invoice:
                    break
            if not invoice:
                invoice = self.env['cncw.invoice.move'].create({})
    ...
    def _create_invoice_from_single_attachment(self, attachment):
        ...
        return self.env['cncw.invoice.move'].browse(invoice_action['res_id'])
    ...
    def _get_journal_bank_account_balance(self, domain=None):
        ...
        '''
        self.ensure_one()
        self.env['cncw.invoice.move.line'].check_access_rights('read')
        ...
        query = self.env['cncw.invoice.move.line']._where_calc(domain)
        tables, where_clause, where_params = query.get_sql()

        query = '''
            SELECT
                COUNT(cncw_invoice_move_line.id) AS nb_lines,
                COALESCE(SUM(cncw_invoice_move_line.balance), 0.0),
                COALESCE(SUM(cncw_invoice_move_line.amount_currency), 0.0)
            FROM ''' + tables + '''
            WHERE ''' + where_clause + '''
        '''
    ...
    def _get_journal_outstanding_payments_account_balance(self, domain=None, date=None):
        ...
        self._cr.execute('''
            SELECT
                COUNT(cncw_invoice_move_line.id) AS nb_lines,
                cncw_invoice_move_line.currency_id,
                account.reconcile AS is_account_reconcile,
                SUM(cncw_invoice_move_line.amount_residual) AS amount_residual,
                SUM(cncw_invoice_move_line.balance) AS balance,
                SUM(cncw_invoice_move_line.amount_residual_currency) AS amount_residual_currency,
                SUM(cncw_invoice_move_line.amount_currency) AS amount_currency
            FROM ''' + tables + '''
            JOIN cncw_account account ON account.id = cncw_invoice_move_line.account_id
            WHERE ''' + where_clause + '''
            GROUP BY cncw_invoice_move_line.currency_id, account.reconcile
        ''', where_params)
