...
class AccountPayableReport(models.Model):
    ...
    partner_code = fields.Char('供应商编码')
    partner_id = fields.Many2one('res.partner', '供应商名称')
    categ_id = fields.Many2one('product.category', '货品分类')
    partner_name = fields.Char('供应商名称')
    overdue_amount = fields.Float('超期应收款',  digits='Product Price', aggregator="sum")
    overdue_days = fields.Float('超期天数', digits=(16, 0))
    payment_term_id = fields.Many2one('account.payment.term', string=u"收款款条件")
    overdue_time_range = fields.Selection(OVER_DATA_SELECTION, string='超期范围')
    ...
    date_due = fields.Date('应付款日期')
    payment_amount = fields.Float('付款金额',  digits='Product Price', aggregator="sum")
    remaining_amount = fields.Float('剩余应付款',  digits='Product Price', aggregator="sum")
    ...
    def _search_user_id(self, operation, value):
        ...
        list = base_cw.public.get_user_by_partner(self, self.partner_id.id)
        if list:
            self._cr.execute("""select id from account_payable_report where partner_id in %s""", (tuple(list),))
            order = filter(None, map(lambda x: x[0], self._cr.fetchall()))
        return [('id', 'in', order)]

    def init(self):
        tools.drop_view_if_exists(self._cr, 'account_payable_report')
        self._cr.execute("""create or replace view account_payable_report as (
                select t1.id,t1.partner_id,t2.code as partner_code,t2.name partner_name,t1.categ_id,--t6.manager_id,,t3.user_id,t9.execution_user_id
                        ...
                        (case when t1.remaining_amount=0 then '已全部付款' else '未全部付款' end) as invoice_state,--t5.department_id,
